...
public class FYVideoCompressor {
  ...
  public enum VideoQuality: Equatable {
    ...
    var value: (fps: Float, bitrate: Int) {
      ...
      case .highQuality:
        return (30, 8000_000)
      case .custom(let fps, let bitrate, _):
        return (fps, bitrate)
      }
    }
  ...
  @available(
    *, deprecated, renamed: "init()",
    message:
      "In the case of batch compression, singleton causes a crash, be sure to use init method - init()"
  )
  ...
  public func compressVideo(
    _ url: URL, quality: VideoQuality = .mediumQuality, outputPath: URL? = nil,
    ...
    // --- Video ---
    // video bit rate
    let targetVideoBitrate = getVideoBitrateWithQuality(
      quality, originalBitrate: videoTrack.estimatedDataRate)

    // scale size
    let scaleSize = calculateSizeWithQuality(quality, originalSize: videoTrack.naturalSize)

    let videoSettings = createVideoSettingsWithBitrate(
      targetVideoBitrate,
    ...
    if let adTrack = asset.tracks(withMediaType: .audio).first {
      ...
      audioSettings = createAudioSettingsWithAudioTrack(
        adTrack, bitrate: Float(audioBitrate), sampleRate: audioSampleRate)
    }
    #if DEBUG
      print("🎬 Video ")
      print("ORIGINAL:")
  ...
  public func compressVideo(
    _ url: URL, config: CompressionConfig, completion: @escaping (Result<URL, Error>) -> Void
  ) {
    let asset = AVAsset(url: url)
    // setup
    guard let videoTrack = asset.tracks(withMediaType: .video).first else {
    ...
    if let adTrack = asset.tracks(withMediaType: .audio).first {
      ...
      audioSettings = createAudioSettingsWithAudioTrack(
        adTrack, bitrate: targetAudioBitrate, sampleRate: targetSampleRate)
    }
  ...
  private func _compress(
    ...
    // video
    let videoOutput = AVAssetReaderTrackOutput.init(
      track: videoTrack,
      outputSettings: [
    ...
    do {
      ...
      if reduceFPS {
        outputVideoDataByReducingFPS(
          originFPS: videoTrack.nominalFrameRate,
          targetFPS: targetFPS,
  ...
  private func createVideoSettingsWithBitrate(
    _ bitrate: Float, maxKeyFrameInterval: Int, size: CGSize
  ) -> [String: Any] {
    return [
  ...
  private func createAudioSettingsWithAudioTrack(
    _ audioTrack: AVAssetTrack, bitrate: Float, sampleRate: Int
  ) -> [String: Any] {
    #if DEBUG
      if let audioFormatDescs = audioTrack.formatDescriptions as? [CMFormatDescription],
        let formatDescription = audioFormatDescs.first
      {
        print("🔊 Audio")
        print("ORINGIAL:")
        print("bitrate: \(audioTrack.estimatedDataRate)")
        if let streamBasicDescription = CMAudioFormatDescriptionGetStreamBasicDescription(
          formatDescription)
        {
    ...
    return [
      ...
      AVChannelLayoutKey: Data(
        bytes: &audioChannelLayout, count: MemoryLayout<AudioChannelLayout>.size),
    ]
  }

  private func outputVideoDataByReducingFPS(
    originFPS: Float,
    targetFPS: Float,
    ...
    let randomFrames = getFrameIndexesWith(
      originalFPS: originFPS, targetFPS: targetFPS, duration: Float(duration.seconds))
    var counter = 0
    var index = 0
        ...
        if let buffer = videoOutput.copyNextSampleBuffer() {
          ...
          if index < randomFrames.count {
            ...
            if counter == frameIndex {
              ...
              timingInfo.pointee.duration = CMTimeMultiplyByFloat64(
                timingInfo.pointee.duration, multiplier: Float64(originFPS / targetFPS))

              // Again, should check call succeeded
              CMSampleBufferCreateCopyWithNewTiming(
                allocator: nil, sampleBuffer: buffer, sampleTimingEntryCount: 1,
