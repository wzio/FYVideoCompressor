...
final class FYVideoCompressorTests: XCTestCase {
  ...
  static let testVideoURL = URL(
    string: "https://www.learningcontainer.com/wp-content/uploads/2020/05/sample-mov-file.mov")!

  let sampleVideoPath: URL = FileManager.tempDirectory(with: "UnitTestSampleVideo")
  ...
  func testTargetVideoSizeWithQuality() {
    let targetSize = compressor.calculateSizeWithQuality(
      .lowQuality, originalSize: CGSize(width: 1920, height: 1080))
    XCTAssertEqual(targetSize, CGSize(width: 398, height: 224))
  }

  func testTargetVideoSizeWithConfig() {
    let scale1 = compressor.calculateSizeWithScale(
      CGSize(width: -1, height: 224), originalSize: CGSize(width: 1920, height: 1080))
    XCTAssertEqual(scale1, CGSize(width: 398, height: 224))

    let scale2 = compressor.calculateSizeWithScale(
      CGSize(width: 640, height: -1), originalSize: CGSize(width: 1920, height: 1080))
        ...
        case .success(let data):
          do {
            try (data as NSData).write(
              to: self.sampleVideoPath, options: NSData.WritingOptions.atomic)
            completion(.success(self.sampleVideoPath))
          } catch {
