...
class TestUserDefinedPath: XCTestCase {
  ...
  func testCompressVideo() {
    ...
    XCTAssertTrue(
      FileManager.default.isValidDirectory(atPath: compressedVideoPath),
      "user defined path is invalid")

    compressor.compressVideo(sampleVideoPath, quality: .lowQuality, outputPath: compressedVideoPath)
    { result in
      switch result {
